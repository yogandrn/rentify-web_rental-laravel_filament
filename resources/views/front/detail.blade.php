...
<section id="Thumbnail" class="flex relative h-[370px] pt-[94px] pb-[66px] bg-[#F6F6F6]">
	...
	<div class="flex gap-4 absolute -bottom-[35px] w-full items-center justify-center">
		<button
			...
			<img src="{{ Storage::url($product->thumbnail) }}" alt="{{$product->name}}-picture" class="size-full" />
		</button>
		@forelse ($product->pictures as $picture)
			<button
				class="thumbnail-button size-[70px] flex p-[15px_20px] rounded-full overflow-hidden bg-white flex-shrink-0 ">
				<img src="{{ Storage::url($picture->url_asset) }}" alt="{{$product->name}}-picture" class="size-full" />
			</button>
...
<div id="Bottom-nav"
	...
	<div class="flex items-center justify-between p-5 relative z-10">
		<div class="flex flex-col gap-1 w-fit">
			<p class="font-bold text-xl leading-[30px]">IDR {{number_format($product->price, 0, ',', '.')}}</p>
			<p class="text-sm leading-[21px]">/day</p>
		</div>
		<a href="{{ route('product.booking', $product->slug) }}" class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-fit">Rent Now</a>
	</div>
</div>
@endsection
