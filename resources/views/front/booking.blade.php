...
@section('content')
		...
		<form action="{{route('product.booking.rent', $product->slug)}}" method="POST" class="flex flex-col gap-[30px] mt-[30px]">
			@csrf
			...
			<div class="flex flex-col px-5 gap-2">
				<label for="date" class="font-semibold">Delivery Type</label>
				...
				<div id="Menus" class="flex flex-col gap-4">
					...
					<div class="tabs-contents">
                        <div id="pickup-tab" class="tabcontent flex flex-col gap-4 hidden">
							<label class="font-semibold">Select Store</label>
							...
							<label
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <p class="text-xs leading-[18px] text-[#6E6E70]">{{ $item->address }}</p>
                                    </div>
                                </div>
                                <input type="radio" name="store_id" id="store_id" value="{{$item->id}}"
                                        class="absolute -z-10 top-1/2 left-1/2 opacity-0" />
                                </label>
                            @empty
                                <label
                            class="relative w-full rounded-2xl p-[18px_14px] border border-[#EDEEF0] transition-all duration-300 hover:ring-2 hover:ring-[#FCCF2F] has-[:checked]:ring-2 has-[:checked]:ring-[#FCCF2F]">
                                <p>Belum ada Store yang bisa dipilih</p>
                            </label>
                            @endforelse
				...
				<div class="flex items-center justify-between px-6 relative mt-5">
					<p class="col">Subtotal</p>
					<p id="subtotal-text" class="col"></p>
				</div>
				<div class="flex items-center justify-between px-6 relative">
					<p class="col">Tax (11%)</p>
					<p id="tax-text" class="col"></p>
				</div>
				<div class="flex items-center justify-between px-6 relative">
					<p class="col">Fee</p>
					<p id="fee-text" class="col"></p>
				</div>
			...
			<div id="Bottom-nav"
				...
				<div class="flex items-center justify-between p-5 relative z-10">
					<div class="flex flex-col gap-1 w-fit">
                        ...
                        <input type="hidden" name="price" id="price" value="{{intval($product->price)}}">
...
@section('script')
	<script src="{{asset("js/on_booking.js")}}"></script>
@endsection