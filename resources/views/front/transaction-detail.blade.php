    ...
    <main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
        ...
        <section class="flex flex-col gap-[30px] mt-[30px] px-5">
            ...
            @if ($transaction->status == 'CANCELED')
                <div id="Payment-canceled" class="flex items-center rounded-2xl p-[14px_16px] gap-4 text-white" style="background-color: #d92c1e">
                    <div class="w-6 h-6 flex shrink-0">
                        <img src="{{asset("assets/images/icons/note-white.svg")}}" alt="icon">
                    </div>
                    <div class="flex flex-col gap-1">
                        <div class="flex items-center gap-1">
                            <p class="font-semibold text-sm leading-[21px]">Booking Canceled</p>
                            {{-- <div class="w-5 h-5 flex shrink-0">
                                <img src="{{asset("assets/images/icons/note-black.svg")}}" alt="icon">
                            </div> --}}
                        </div>
                        <p class="text-xs leading-[18px]">Transaksi dibatalkan karena pembayaran tidak valid atau usudah expired!</p>
                    </div>
                </div>
            @endif
            ...
            <div class="flex items-center gap-[14px]">
                ...
                <div class="w-full flex flex-col gap-2">
                    <p class="font-bold text-lg leading-[27px]">{{ $transaction->product->name }}</p>
                    <div class="flex items-center gap-[14px]">
            ...
            <section class="flex flex-col gap-5">
                <div class="info-card flex flex-col gap-2">
                    ...
                    <div class="flex items-center rounded-2xl p-[18px_14px] gap-3 bg-[#F4F4F6]">
                        ...
                        <div class="flex flex-col gap-1">
                            <p class="font-semibold">{{ $transaction->fullname }}</p>
                        </div>
                    </div>
                </div>
                <div class="info-card flex flex-col gap-2">
                ...
                @if ($transaction->delivery_type == 'PICKUP')
                    <div class="info-card flex flex-col gap-2">
                        ...
                        <div class="flex items-center rounded-2xl p-[18px_14px] gap-3 bg-[#F4F4F6]">
                            ...
                            <div class="flex flex-col gap-1">
                                <p class="font-semibold">{{ $transaction->store->name }}</p>
                                <p class="text-sm leading-[21px] text-[#6E6E70]">{{ $transaction->store->address }}</p>
                            </div>
                        </div>
                    </div>
                @else
            ...
            <div id="Payment-details" class="flex flex-col gap-3">
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex items-center justify-between">
                        <p>Sub total</p>
                        <p class="font-semibold">Rp {{ number_format($transaction->subtotal, 0, ',', '.') }} </p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>PPN 11%</p>
                        <p class="font-semibold">Rp {{ number_format($transaction->tax, 0, ',', '.') }} </p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>Admin Fee</p>
                        <p class="font-semibold">Rp {{ number_format($transaction->fee, 0, ',', '.') }} </p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>Grand total</p>
                        <p class="font-bold text-xl leading-[30px] underline">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }} </p>
                    </div>
                </div>
            </div>
        </section>
