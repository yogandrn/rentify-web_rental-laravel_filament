        ...
        <main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
			<section id="finishBook" class="flex flex-col gap-[30px] max-w-[353px] p-[30px_20px] items-center m-auto">
				...
				<div class="w-full h-[220px] flex items-center justify-center flex-shrink-0">
					<img src="{{ Storage::url($transaction->product->thumbnail) }}" alt="{{ $transaction->product->name }}" class="size-full object-contain" />
				</div>
				...
				<div class="w-[220px] flex flex-col gap-3 items-center">
					<a href="{{ route('home.index')}}" class="w-full text-center rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold text-black">Rent More</a>
					<a href="{{ route('transaction.index') }}" class="w-full text-center rounded-full p-[12px_24px] bg-white font-bold text-black outline outline-1">Booking Details</a>
				</div>
			</section>
		</main>
@endsection