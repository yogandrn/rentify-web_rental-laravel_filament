...
@section('content')
    ...
    <section id="brand"
        ...
        <div class="h-[30px] flex shrink-0">
            <img src="{{ Storage::url($brand->logo) }}" alt="{{$brand->name}}" class="size-full" />
        </div>
        <div class="flex flex-col items-end gap-[2x]">
            <p class="font-semibold">{{ $brand->name }}</p>
            <p class="text-sm leading-[21px] text-[#6E6E70] text-nowrap">{{ count($brand->products) }} products</p>
        </div>
    </section>
    <section id="Explore" class="flex flex-col gap-[10px] mx-5 mt-[30px]">
        <h2 class="text-lg leading-[27px] font-semibold">Explore Gadgets</h2>
        <div class="flex flex-col gap-5">
            ...
            @empty
                <p>Belum ada produk dari brand <b>{{$brand->name}}</b> untuk kategori ini</p>
            @endforelse
